...
async fn running_and_external_forward_exact_original_bytes() {
    ...
    for intent in [1, 2, 3] {
        assert_phase_proxies_exact_bytes(ServerPhase::Running, handshake_frame(protocol, intent))
            .await;
    }
    assert_phase_proxies_exact_bytes(ServerPhase::Running, non_minimal_unknown_handshake()).await;
    assert_phase_proxies_exact_bytes(ServerPhase::External, non_minimal_unknown_handshake()).await;
}
...
async fn login_session_is_established_only_after_complete_replay() {
    ...
    let session = establish_owned_proxy_activity(&context, cycle, HandshakeIntent::Login, lease)
...
async fn sleeping_login_transfer_and_cooldown_queue_wakes() {
    for (phase, intent) in [
        ...
        let (context, _lifecycle_authority) = client_context_with_lifecycle(phase, wake_sender, 9);
...
async fn bind_failure_has_no_runtime_work_to_clean_up() {
    ...
    let prepared = ServerRuntime::prepare(config, test_responder(), Arc::new(Semaphore::new(1)))
...
async fn shutdown_watchdog_is_validated_before_listener_binding() {
    ...
    let Err(error) = ServerRuntime::prepare(config, test_responder(), Arc::new(Semaphore::new(1)))
    else {
